...
function openEditAccountTable(accountId) {
  ...
  // Lấy thông tin từ user mặc định
  accountName.value = currentUser.username;
  accountPassword.value = currentUser.password;
  accountMail.value = currentUser.email;
  accountTel.value = currentUser.phone;
  idAccount.value = currentUser.id;
}

function editAccountToAccountArray(event) {
  event.preventDefault();
  if(ValidateEmail(accountMail.value) && validatePhone(accountTel.value)) {
    alert("Cập nhật tài khoản thành công!");
    closeEditAccountTable();
  } else {
